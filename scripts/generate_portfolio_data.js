...
function scanAndBuildData() {
  ...
  const getProjectWeight = (proj) => {
    const idx = projectOrder.indexOf(proj);
  ...
  const sortedItems = items.sort((a, b) => {
    ...
    const weightA = getProjectWeight(a.project);
    const weightB = getProjectWeight(b.project);
  ...
  const uniqueProjectsList = Array.from(new Set(normalizedDirs)).sort((a, b) => {
    const weightA = getProjectWeight(a);
    const weightB = getProjectWeight(b);
